...
static void test_scaled_clip_geglu(h3_gpu *gpu) {
    ...
    if (!failed) {
        ...
        CHECK(check_f32(got, expected_scaled, N, 1e-7f, "add_scaled"));
        CHECK(h3_gpu_tensor_read_f32(clipped, got, N) == 1);
        CHECK(check_f32(got, expected_clip, N, 1e-7f, "clip"));
        CHECK(h3_gpu_tensor_read_f32(geglu, got, N) == 1);
        CHECK(check_f32(got, expected_geglu, N, 1e-6f, "geglu"));
    }
...
static void test_euler(h3_gpu *gpu) {
    ...
    if (!failed) {
        ...
        CHECK(check_f32(got, expected, N + OFFSET, 1e-7f, "euler"));
    }
...
static void test_embedding(h3_gpu *gpu) {
    ...
    for (size_t token = 0; token < TOKENS; token++) {
        ids[token] = (uint32_t)((token % 7 == 0) ? VOCAB + token :
                                token * 17 % VOCAB);
...
static void test_norms(h3_gpu *gpu) {
    ...
    if (!failed) {
        ...
        CHECK(check_f32(got, expected_rms, ROWS * WIDTH, 1e-4f, "rms_f32"));
        CHECK(h3_gpu_tensor_read_f32(layer, got, ROWS * WIDTH) == 1);
        CHECK(check_f32(got, expected_layer, ROWS * WIDTH, 1e-4f,
                        "layer_f32"));
...
static void test_grouped_qkv_linear_rope_bf16(h3_gpu *gpu) {
    enum { ROWS = 4, INPUT_DIM = 9, HEADS = 3, DIM = 8, HALF = 4 };
    const float epsilon = 1e-5f;
    size_t inner = (size_t)HEADS * DIM;
    size_t weight_count = inner * 3 * INPUT_DIM;
    uint16_t input[ROWS * INPUT_DIM], weight[weight_count];
    uint16_t qn[DIM], kn[DIM], rcos[ROWS * HALF], rsin[ROWS * HALF];
    for (size_t index = 0; index < ROWS * INPUT_DIM; index++)
        input[index] = bf16_bits((float)(sin((double)index * 0.41) * 1.2));
    for (size_t index = 0; index < weight_count; index++)
        weight[index] = bf16_bits((float)(cos((double)index * 0.57) * 0.3));
    ...
    /* reference: plain projection, then grouped norm/RoPE */
    uint16_t qkv[ROWS * HEADS * 3 * DIM];
    linear_reference(input, weight, NULL, 0, qkv, ROWS, INPUT_DIM,
                     (uint32_t)(inner * 3));
    uint16_t expected_q[ROWS * HEADS * DIM], expected_k[ROWS * HEADS * DIM];
    for (uint32_t row = 0; row < ROWS; row++) {
    ...
    h3_gpu_tensor *qkv_t = h3_gpu_tensor_new_bf16(gpu, ROWS * HEADS * 3 * DIM);
    h3_gpu_tensor *oq = h3_gpu_tensor_new_bf16(gpu, ROWS * HEADS * DIM);
    h3_gpu_tensor *ok = h3_gpu_tensor_new_bf16(gpu, ROWS * HEADS * DIM);
    h3_gpu_tensor *ov = h3_gpu_tensor_new_bf16(gpu, ROWS * HEADS * DIM);
    CHECK(in && w && q && k && c && s && qkv_t && oq && ok && ov);
    if (!failed) {
        h3_gpu_begin(gpu);
    ...
    h3_gpu_tensor_free(s);
    h3_gpu_tensor_free(qkv_t);
    h3_gpu_tensor_free(oq);
    h3_gpu_tensor_free(ok);
    h3_gpu_tensor_free(ov);
}

static void test_mlp_nax_bf16(h3_gpu *gpu) {
    enum { ROWS = 8, INPUT_DIM = 16, HIDDEN = 12, OUTPUT_DIM = 20 };
    size_t fc1_count = (size_t)HIDDEN * 2 * INPUT_DIM;
    size_t fc2_count = (size_t)OUTPUT_DIM * HIDDEN;
    uint16_t input[ROWS * INPUT_DIM], fc1_w[fc1_count], fc2_w[fc2_count];
    ...
    h3_gpu_tensor *w1 = h3_gpu_tensor_from_bf16(gpu, fc1_w, fc1_count);
    h3_gpu_tensor *w2 = h3_gpu_tensor_from_bf16(gpu, fc2_w, fc2_count);
    h3_gpu_tensor *act_t = h3_gpu_tensor_new_bf16(gpu, ROWS * HIDDEN);
    h3_gpu_tensor *out = h3_gpu_tensor_new_bf16(gpu, ROWS * OUTPUT_DIM);
    CHECK(in && w1 && w2 && act_t && out);
    if (!failed) {
        h3_gpu_begin(gpu);
        ...
        if (memcmp(got, expected, sizeof(got)) != 0) {
            for (size_t index = 0; index < ROWS * OUTPUT_DIM; index++) {
                if (got[index] != expected[index]) {
                    fprintf(stderr, "  mlp nax[%zu]: got %u expected %u\n",
                            index, got[index], expected[index]);
                    break;
                }
            }
    ...
    h3_gpu_tensor_free(w1);
    h3_gpu_tensor_free(w2);
    h3_gpu_tensor_free(act_t);
    h3_gpu_tensor_free(out);
}

static void test_swiglu_f32(h3_gpu *gpu) {
    uint32_t rows = 7, width = 33;
    size_t count = (size_t)rows * width;
    float *fused = malloc(count * 2 * sizeof(float));
    float *expected = malloc(count * sizeof(float));
    float *got = malloc(count * sizeof(float));
    for (size_t index = 0; index < count * 2; index++)
        fused[index] = (float)(sin((double)index * 0.63) * 1.5);
    for (uint32_t row = 0; row < rows; row++) {
        for (uint32_t column = 0; column < width; column++) {
            float gate = fused[row * width * 2 + column];
            float up = fused[row * width * 2 + width + column];
            expected[row * width + column] =
                gate / (1.0f + expf(-gate)) * up;
        }
    }
    h3_gpu_tensor *in = h3_gpu_tensor_from_f32(gpu, fused, count * 2);
    h3_gpu_tensor *out = h3_gpu_tensor_new_f32(gpu, count);
    CHECK(in && out);
    if (!failed) {
        h3_gpu_begin(gpu);
        ...
        CHECK(h3_gpu_submit(gpu) == 1);
        CHECK(h3_gpu_tensor_read_f32(out, got, count) == 1);
        CHECK(check_f32(got, expected, count, 1e-6f, "swiglu_f32"));
    }
    h3_gpu_tensor_free(in);
    h3_gpu_tensor_free(out);
    free(fused); free(expected); free(got);
...
static void test_video_qkv_rope_f32(h3_gpu *gpu) {
    enum { SEQ = 4, HEADS = 3, DIM = 8, HALF = 4 };
    const float epsilon = 1e-5f;
    float qkv[SEQ * HEADS * 3 * DIM], rcos[SEQ * HALF], rsin[SEQ * HALF];
    float expected_q[SEQ * HEADS * DIM], expected_k[SEQ * HEADS * DIM];
    for (size_t index = 0; index < SEQ * HEADS * 3 * DIM; index++)
        qkv[index] = (float)(sin((double)index * 0.37) * 1.5);
    for (size_t index = 0; index < SEQ * HALF; index++) {
        rcos[index] = cosf((float)index * 0.31f);
        rsin[index] = sinf((float)index * 0.31f);
    }
    for (uint32_t row = 0; row < SEQ; row++) {
        for (uint32_t head = 0; head < HEADS; head++) {
            uint32_t base = (row * HEADS + head) * DIM * 3;
            float q_sum = 0.0f, k_sum = 0.0f;
            for (uint32_t d = 0; d < DIM; d++) {
                q_sum = fmaf(qkv[base + d], qkv[base + d], q_sum);
                k_sum = fmaf(qkv[base + DIM + d], qkv[base + DIM + d], k_sum);
            }
            float qi = 1.0f / sqrtf(q_sum / (float)DIM + epsilon);
            float ki = 1.0f / sqrtf(k_sum / (float)DIM + epsilon);
            for (uint32_t d = 0; d < DIM; d++) {
                float q0 = qkv[base + d] * qi;
                float k0 = qkv[base + DIM + d] * ki;
                if (d < HALF) {
                    float q1 = qkv[base + d + HALF] * qi;
                    float k1 = qkv[base + DIM + d + HALF] * ki;
                    ...
                    float k1 = qkv[base + DIM + pair] * ki;
                    float c = rcos[row * HALF + pair];
                    float s = rsin[row * HALF + pair];
                    q0 = q0 * c + q1 * s;
                    k0 = k0 * c + k1 * s;
                }
                uint32_t index = (row * HEADS + head) * DIM + d;
    ...
    h3_gpu_tensor *t = h3_gpu_tensor_from_f32(gpu, qkv, SEQ * HEADS * 3 * DIM);
    h3_gpu_tensor *c = h3_gpu_tensor_from_f32(gpu, rcos, SEQ * HALF);
    h3_gpu_tensor *s = h3_gpu_tensor_from_f32(gpu, rsin, SEQ * HALF);
    h3_gpu_tensor *oq = h3_gpu_tensor_new_f32(gpu, SEQ * HEADS * DIM);
    h3_gpu_tensor *ok = h3_gpu_tensor_new_f32(gpu, SEQ * HEADS * DIM);
    h3_gpu_tensor *ov = h3_gpu_tensor_new_f32(gpu, SEQ * HEADS * DIM);
    ...
    if (!failed) {
        ...
        CHECK(check_f32(got, expected_q, SEQ * HEADS * DIM, 1e-4f,
                        "video_qkv_q"));
        CHECK(h3_gpu_tensor_read_f32(ok, got, SEQ * HEADS * DIM) == 1);
        CHECK(check_f32(got, expected_k, SEQ * HEADS * DIM, 1e-4f,
                        "video_qkv_k"));
    }
    h3_gpu_tensor_free(t);
    h3_gpu_tensor_free(c);
...
static void test_vae_encoder_pad_f32(h3_gpu *gpu) {
    enum { BATCH = 2, DEPTH = 3, HEIGHT = 4, WIDTH = 5, CH = 2 };
    enum { DF = 1, HB = 1, HA = 2, WB = 2, WA = 1 };
    size_t input_count = (size_t)BATCH * DEPTH * HEIGHT * WIDTH * CH;
    size_t out_depth = DEPTH + DF;
    size_t out_height = HEIGHT + HB + HA;
    size_t out_width = WIDTH + WB + WA;
    size_t output_count = (size_t)BATCH * out_depth * out_height * out_width * CH;
    float *input = malloc(input_count * sizeof(float));
    float *expected = malloc(output_count * sizeof(float));
    float *got = malloc(output_count * sizeof(float));
    for (size_t index = 0; index < input_count; index++)
        input[index] = (float)(sin((double)index * 0.27) * 2.0);
    for (uint32_t batch = 0; batch < BATCH; batch++) {
        for (uint32_t t = 0; t < out_depth; t++) {
            for (uint32_t y = 0; y < out_height; y++) {
                for (uint32_t x = 0; x < out_width; x++) {
                    for (uint32_t channel = 0; channel < CH; channel++) {
                        size_t destination =
                            (((((size_t)batch * out_depth + t) * out_height +
                               y) * out_width + x) * CH + channel);
                        float value = 0.0f;
                        if (t >= DF) {
                            int source_y = (int)y - (int)HB;
                            if (source_y < 0) source_y = -source_y;
                            if (source_y >= (int)HEIGHT)
                                source_y = 2 * (int)HEIGHT - source_y - 2;
                            int source_x = (int)x - (int)WB;
                            if (source_x < 0) source_x = -source_x;
                            if (source_x >= (int)WIDTH)
                                source_x = 2 * (int)WIDTH - source_x - 2;
                            uint32_t source_t = t - DF;
                            size_t source =
                                (((((size_t)batch * DEPTH + source_t) *
                                   HEIGHT + (uint32_t)source_y) * WIDTH +
                                  (uint32_t)source_x) * CH + channel);
                            value = input[source];
                        }
    ...
    if (!failed) {
        ...
        if (memcmp(got, expected, output_count * sizeof(float)) != 0) {
            for (size_t index = 0; index < output_count; index++) {
                if (got[index] != expected[index]) {
                    fprintf(stderr, "  vae pad[%zu]: got %.9g expected %.9g\n",
                            index, got[index], expected[index]);
                    break;
                }
            }
    ...
    h3_gpu_tensor_free(in);
    h3_gpu_tensor_free(out);
    free(input); free(expected); free(got);
}

static void test_vae_group_norm_silu_f32(h3_gpu *gpu) {
    enum { BATCH = 2, DEPTH = 2, HEIGHT = 3, WIDTH = 4, CH = 4, GROUPS = 2 };
    const float epsilon = 1e-5f;
    size_t count = (size_t)BATCH * DEPTH * HEIGHT * WIDTH * CH;
    size_t rows = (size_t)BATCH * DEPTH * GROUPS;
    size_t channels_per_group = CH / GROUPS;
    size_t elements = (size_t)HEIGHT * WIDTH * channels_per_group;
    float *input = malloc(count * sizeof(float));
    float *weight = malloc(CH * sizeof(float));
    float *bias = malloc(CH * sizeof(float));
    float *expected = malloc(count * sizeof(float));
    float *got = malloc(count * sizeof(float));
    for (size_t index = 0; index < count; index++)
        input[index] = (float)(sin((double)index * 0.53) * 2.0);
    for (size_t index = 0; index < CH; index++) {
        weight[index] = (float)cos((double)index * 0.19) * 0.5f + 1.0f;
        bias[index] = (float)sin((double)index * 0.11) * 0.25f;
    }
    for (uint32_t row = 0; row < rows; row++) {
        ...
        for (uint32_t index = 0; index < elements; index++) {
            ...
            size_t channel = (size_t)group_index * channels_per_group +
                             index % channels_per_group;
            size_t source = ((size_t)temporal_plane * HEIGHT * WIDTH +
                             spatial) * CH + channel;
            values[index] = input[source];
        }
        float mean = tree_sum256(values, (uint32_t)elements) / (float)elements;
        float *squared = malloc(elements * sizeof(float));
        for (uint32_t index = 0; index < elements; index++) {
            float centered = values[index] - mean;
            squared[index] = fmaf(centered, centered, 0.0f);
        }
        float inverse = 1.0f / sqrtf(tree_sum256(squared, (uint32_t)elements) /
        ...
        for (uint32_t index = 0; index < elements; index++) {
            ...
            size_t channel = (size_t)group_index * channels_per_group +
                             index % channels_per_group;
            size_t destination =
                ((size_t)temporal_plane * HEIGHT * WIDTH + spatial) * CH +
                channel;
            float value = (values[index] - mean) * inverse * weight[channel] +
                          bias[channel];
            expected[destination] = value / (1.0f + expf(-value));
        }
        free(values);
        free(squared);
    }
    h3_gpu_tensor *in = h3_gpu_tensor_from_f32(gpu, input, count);
    h3_gpu_tensor *w = h3_gpu_tensor_from_f32(gpu, weight, CH);
    h3_gpu_tensor *b = h3_gpu_tensor_from_f32(gpu, bias, CH);
    h3_gpu_tensor *out = h3_gpu_tensor_new_f32(gpu, count);
    CHECK(in && w && b && out);
    if (!failed) {
        h3_gpu_begin(gpu);
        ...
        CHECK(h3_gpu_submit(gpu) == 1);
        CHECK(h3_gpu_tensor_read_f32(out, got, count) == 1);
        CHECK(check_f32(got, expected, count, 1e-4f, "vae_gn_silu"));
    }
    h3_gpu_tensor_free(in);
    h3_gpu_tensor_free(w);
    h3_gpu_tensor_free(b);
...
static void test_conv3d_f32(h3_gpu *gpu) {
    enum { BATCH = 2, DEPTH = 4, HEIGHT = 5, WIDTH = 6, IC = 3, OC = 4 };
    enum { KD = 2, KH = 3, KW = 2, SD = 1, SH = 2, SW = 1 };
    size_t input_count = (size_t)BATCH * DEPTH * HEIGHT * WIDTH * IC;
    size_t weight_count = (size_t)OC * IC * KD * KH * KW;
    size_t output_depth = (DEPTH - KD) / SD + 1;
    size_t output_height = (HEIGHT - KH) / SH + 1;
    size_t output_width = (WIDTH - KW) / SW + 1;
            ...
            for (uint32_t y = 0; y < output_height; y++) {
                for (uint32_t x = 0; x < output_width; x++) {
                    for (uint32_t oc = 0; oc < OC; oc++) {
                        ...
                        size_t output_index =
                            ((((size_t)batch * output_depth + t) *
                              output_height + y) * output_width + x) * OC +
                            oc;
    ...
    if (!failed) {
        ...
        if (memcmp(got, expected, output_count * sizeof(float)) != 0) {
            for (size_t index = 0; index < output_count; index++) {
                if (got[index] != expected[index]) {
                    fprintf(stderr, "  conv3d[%zu]: got %.9g expected %.9g\n",
                            index, got[index], expected[index]);
                    break;
                }
            }
...
static void test_weight_norm_f32(h3_gpu *gpu) {
    ...
    if (!failed) {
        ...
        CHECK(h3_gpu_submit(gpu) == 1);
        CHECK(h3_gpu_tensor_read_f32(out, got, count) == 1);
        CHECK(check_f32(got, expected, count, 1e-4f, "weight_norm"));
    }
    h3_gpu_tensor_free(v);
...
static void test_snake1d_f32(h3_gpu *gpu) {
    ...
    if (!failed) {
        ...
        CHECK(h3_gpu_submit(gpu) == 1);
        CHECK(h3_gpu_tensor_read_f32(out, got, count) == 1);
        CHECK(check_f32(got, expected, count, 1e-6f, "snake1d"));
    }
    h3_gpu_tensor_free(in);
    h3_gpu_tensor_free(a);
    h3_gpu_tensor_free(out);
...
static void test_alias_free_snake_f32(h3_gpu *gpu) {
    ...
    if (!failed) {
        ...
        CHECK(h3_gpu_submit(gpu) == 1);
        CHECK(h3_gpu_tensor_read_f32(out, got, count) == 1);
        CHECK(check_f32(got, expected, count, 1e-4f, "alias_free_snake"));
    }
    h3_gpu_tensor_free(in);
...
static void int8_quantize_reference(const uint16_t *input, int8_t *output,
                                    float *scales, uint32_t rows,
                                    uint32_t columns, float clip) {
    for (uint32_t row = 0; row < rows; row++) {
        float max_abs = 0.0f;
        for (uint32_t column = 0; column < columns; column++) {
            float value = bf16_value(input[row * columns + column]);
            if (fabsf(value) > max_abs) max_abs = fabsf(value);
        }
        float clipped = max_abs * clip;
        float scale = clipped > 0.0f ? clipped / 127.0f : 1.0f / 127.0f;
        float inverse = clipped > 0.0f ? 127.0f / clipped : 127.0f;
        scales[row] = scale;
        for (uint32_t column = 0; column < columns; column++) {
            int q = (int)rintf(bf16_value(input[row * columns + column]) *
                               inverse);
            if (q < -127) q = -127;
            if (q > 127) q = 127;
            output[row * columns + column] = (int8_t)q;
        }
    }
}

static void test_quantize_int8(h3_gpu *gpu) {
    enum { ROWS = 5, COLS = 41 };
    uint16_t input[ROWS * COLS];
    int8_t expected[ROWS * COLS];
    float expected_scales[ROWS];
    int8_t got[ROWS * COLS];
    float got_scales[ROWS];
    for (size_t index = 0; index < ROWS * COLS; index++)
        input[index] = bf16_bits((float)(sin((double)index * 0.37) * 2.0));
    int8_quantize_reference(input, expected, expected_scales, ROWS, COLS,
                            1.0f);
    h3_gpu_tensor *in = h3_gpu_tensor_from_bf16(gpu, input, ROWS * COLS);
    h3_gpu_tensor *out = h3_gpu_tensor_new_i8(gpu, ROWS * COLS);
    h3_gpu_tensor *sc = h3_gpu_tensor_new_f32(gpu, ROWS);
    CHECK(in && out && sc);
    if (!failed) {
        h3_gpu_begin(gpu);
        ...
        if (memcmp(got, expected, ROWS * COLS) != 0) {
            for (size_t index = 0; index < ROWS * COLS; index++) {
                if (got[index] != expected[index]) {
                    fprintf(stderr, "  quantize[%zu]: got %d expected %d\n",
                            index, got[index], expected[index]);
                    break;
                }
            }
...
static void test_linear_int8(h3_gpu *gpu) {
    ...
    int8_t qweight[weight_count];
    float weight_scales[OUTPUT_DIM];
    int8_t qinput[input_count];
    float input_scales[ROWS];
    uint16_t expected[ROWS * OUTPUT_DIM];
    for (size_t index = 0; index < input_count; index++)
        input[index] = bf16_bits((float)(sin((double)index * 0.41) * 1.7));
    for (size_t index = 0; index < weight_count; index++)
        weight[index] = bf16_bits((float)(cos((double)index * 0.53) * 0.6));
    int8_quantize_reference(weight, qweight, weight_scales, OUTPUT_DIM,
                            INPUT_DIM, 1.0f);
    ...
    for (uint32_t row = 0; row < ROWS; row++) {
        for (uint32_t column = 0; column < OUTPUT_DIM; column++) {
            int acc = 0;
            for (uint32_t k = 0; k < INPUT_DIM; k++)
                acc += (int)qinput[row * INPUT_DIM + k] *
                       (int)qweight[column * INPUT_DIM + k];
            float dequant = (float)acc * input_scales[row] *
                            weight_scales[column];
            expected[row * OUTPUT_DIM + column] = bf16_bits(dequant);
        }
    }
    h3_gpu_tensor *in = h3_gpu_tensor_from_bf16(gpu, input, input_count);
    h3_gpu_tensor *w = h3_gpu_tensor_from_bf16(gpu, weight, weight_count);
    ...
    h3_gpu_tensor *ws = h3_gpu_tensor_new_f32(gpu, OUTPUT_DIM);
    CHECK(in && w && qi && is && out && wq && ws);
    if (!failed) {
        h3_gpu_begin(gpu);
        ...
        if (memcmp(got, expected, sizeof(got)) != 0) {
            for (size_t index = 0; index < ROWS * OUTPUT_DIM; index++) {
                if (got[index] != expected[index]) {
                    fprintf(stderr,
                            "  linear int8[%zu]: got %u expected %u\n",
                            index, got[index], expected[index]);
                    break;
                }
            }
...
static void test_linear_int8_head_major(h3_gpu *gpu) {
    enum { ROWS = 3, HEADS = 4, HEAD_DIM = 8, INPUT_DIM = 32,
           OUTPUT_DIM = 13 };
    size_t input_count = (size_t)ROWS * INPUT_DIM;
    size_t weight_count = (size_t)OUTPUT_DIM * INPUT_DIM;
    uint16_t row_input[input_count], head_input[input_count];
    uint16_t weight[weight_count];
    int8_t qinput[input_count], qweight[weight_count];
    float input_scales[ROWS], weight_scales[OUTPUT_DIM];
    uint16_t expected[ROWS * OUTPUT_DIM];
    for (uint32_t row = 0; row < ROWS; row++) {
        for (uint32_t head = 0; head < HEADS; head++) {
            for (uint32_t dimension = 0; dimension < HEAD_DIM; dimension++) {
                uint32_t column = head * HEAD_DIM + dimension;
                uint16_t value = bf16_bits((float)sin(
                    (double)(row * INPUT_DIM + column) * 0.37) * 1.7f);
                row_input[row * INPUT_DIM + column] = value;
                head_input[(head * ROWS + row) * HEAD_DIM + dimension] = value;
            }
        }
    }
    for (size_t index = 0; index < weight_count; index++)
        weight[index] = bf16_bits((float)cos((double)index * 0.29) * 0.6f);
    int8_quantize_reference(row_input, qinput, input_scales, ROWS, INPUT_DIM,
                            1.0f);
    int8_quantize_reference(weight, qweight, weight_scales, OUTPUT_DIM,
                            INPUT_DIM, 1.0f);
    for (uint32_t row = 0; row < ROWS; row++) {
        for (uint32_t column = 0; column < OUTPUT_DIM; column++) {
            ...
            for (uint32_t k = 0; k < INPUT_DIM; k++)
                acc += (int)qinput[row * INPUT_DIM + k] *
                       (int)qweight[column * INPUT_DIM + k];
            expected[row * OUTPUT_DIM + column] = bf16_bits(
                (float)acc * input_scales[row] * weight_scales[column]);
        }
    }
    h3_gpu_tensor *in = h3_gpu_tensor_from_bf16(gpu, head_input, input_count);
    h3_gpu_tensor *w = h3_gpu_tensor_from_bf16(gpu, weight, weight_count);
    h3_gpu_tensor *qi = h3_gpu_tensor_new_i8(gpu, input_count);
    h3_gpu_tensor *is = h3_gpu_tensor_new_f32(gpu, ROWS);
    h3_gpu_tensor *out = h3_gpu_tensor_new_bf16(gpu, ROWS * OUTPUT_DIM);
    ...
    if (!failed) {
        ...
        CHECK(h3_gpu_linear_int8_head_major_bf16(
                  gpu, out, qi, is, in, wq, ws, ROWS, HEADS, HEAD_DIM,
                  OUTPUT_DIM) == 1);
        CHECK(h3_gpu_submit(gpu) == 1);
        uint16_t got[ROWS * OUTPUT_DIM];
        CHECK(h3_gpu_tensor_read_bf16(out, got, ROWS * OUTPUT_DIM) == 1);
...
static void test_mlp_int8(h3_gpu *gpu) {
    ...
    float input_scales[ROWS], act_scales[ROWS];
    uint16_t expected[ROWS * OUTPUT_DIM];
    for (size_t index = 0; index < input_count; index++)
    ...
    int8_quantize_reference(input, qinput, input_scales, ROWS, INPUT_DIM,
                            1.0f);
    /* reference chain: fc1 int8 -> swiglu -> quantize -> fc2 int8 */
    ...
    for (uint32_t row = 0; row < ROWS; row++) {
        for (uint32_t column = 0; column < HIDDEN; column++) {
            ...
            for (uint32_t k = 0; k < INPUT_DIM; k++) {
                ...
                acc_gate += value *
                            (int)qfc1[column * INPUT_DIM + k];
                acc_up += value *
                          (int)qfc1[(HIDDEN + column) * INPUT_DIM + k];
            }
            float gate = bf16_value(bf16_bits(
                (float)acc_gate * input_scales[row] * fc1_scales[column]));
    ...
    int8_quantize_reference(activated, qact, act_scales, ROWS, HIDDEN, 1.0f);
    for (uint32_t row = 0; row < ROWS; row++) {
        for (uint32_t column = 0; column < OUTPUT_DIM; column++) {
            int acc = 0;
            for (uint32_t k = 0; k < HIDDEN; k++)
                acc += (int)qact[row * HIDDEN + k] *
                       (int)qfc2[column * HIDDEN + k];
            float dequant = (float)acc * act_scales[row] *
                            fc2_scales[column];
            expected[row * OUTPUT_DIM + column] = bf16_bits(dequant);
        }
    }
    ...
    h3_gpu_tensor *qa = h3_gpu_tensor_new_i8(gpu, ROWS * INPUT_DIM);
    h3_gpu_tensor *qs = h3_gpu_tensor_new_f32(gpu, ROWS);
    ...
    if (!failed) {
        ...
        if (memcmp(got, expected, sizeof(got)) != 0) {
            for (size_t index = 0; index < ROWS * OUTPUT_DIM; index++) {
                if (got[index] != expected[index]) {
                    fprintf(stderr, "  mlp int8[%zu]: got %u expected %u\n",
                            index, got[index], expected[index]);
                    break;
                }
            }
...
static void test_gate_adaln_quantize_int8(h3_gpu *gpu) {
    enum { ROWS = 4, WIDTH = 128, SLOTS = 2, MAP_ROWS = 2,
           GATE = 0, SHIFT = 1, SCALE = 0 };
    const float epsilon = 1e-5f;
    uint16_t residual[ROWS * WIDTH], branch[ROWS * WIDTH];
    uint16_t gmod[MAP_ROWS * SLOTS * WIDTH], nmod[MAP_ROWS * SLOTS * WIDTH];
    uint16_t weight[WIDTH];
    uint32_t row_map[ROWS];
    int8_t expected[ROWS * WIDTH];
    float expected_scales[ROWS];
    for (size_t index = 0; index < ROWS * WIDTH; index++) {
        residual[index] = bf16_bits((float)(sin((double)index * 0.23) * 1.5));
        branch[index] = bf16_bits((float)(cos((double)index * 0.17) * 1.2));
    }
    for (size_t index = 0; index < MAP_ROWS * SLOTS * WIDTH; index++) {
        gmod[index] = bf16_bits((float)(sin((double)index * 0.11) * 0.4));
        nmod[index] = bf16_bits((float)(cos((double)index * 0.07) * 0.3));
    }
    for (size_t index = 0; index < WIDTH; index++)
        weight[index] = bf16_bits((float)cos((double)index * 0.13) * 0.4f +
                                  1.0f);
    for (size_t index = 0; index < ROWS; index++)
        row_map[index] = index % MAP_ROWS;
    uint16_t gated[ROWS * WIDTH];
    for (uint32_t row = 0; row < ROWS; row++) {
        float values[WIDTH];
        float squares[WIDTH];
        ...
        for (uint32_t column = 0; column < WIDTH; column++) {
            ...
            float gate = bf16_value(gmod[base + GATE * WIDTH + column]);
            uint16_t g = bf16_bits(fmaf(bf16_value(branch[index]), gate,
                                       bf16_value(residual[index])));
            gated[index] = g;
            values[column] = bf16_value(g);
            squares[column] = fmaf(values[column], values[column], 0.0f);
        }
        float inverse = 1.0f / sqrtf(tree_sum256(squares, WIDTH) /
                                     (float)WIDTH + epsilon);
        float max_abs = 0.0f;
        uint16_t normalized_values[WIDTH];
        for (uint32_t column = 0; column < WIDTH; column++) {
            float normalized = values[column] * inverse *
                               bf16_value(weight[column]);
            float shift = bf16_value(nmod[base + SHIFT * WIDTH + column]);
            float scale = bf16_value(nmod[base + SCALE * WIDTH + column]);
            uint16_t value = bf16_bits(fmaf(normalized, 1.0f + scale,
                                           shift));
            normalized_values[column] = value;
            float v = bf16_value(value);
        ...
        expected_scales[row] = output_scale;
        for (uint32_t column = 0; column < WIDTH; column++) {
            int q = (int)rintf(bf16_value(normalized_values[column]) *
                               quantize_scale);
            if (q < -127) q = -127;
            if (q > 127) q = 127;
            expected[row * WIDTH + column] = (int8_t)q;
        }
    }
    h3_gpu_tensor *res = h3_gpu_tensor_from_bf16(gpu, residual, ROWS * WIDTH);
    h3_gpu_tensor *br = h3_gpu_tensor_from_bf16(gpu, branch, ROWS * WIDTH);
    h3_gpu_tensor *w = h3_gpu_tensor_from_bf16(gpu, weight, WIDTH);
    h3_gpu_tensor *gm = h3_gpu_tensor_from_bf16(gpu, gmod,
                                                MAP_ROWS * SLOTS * WIDTH);
    h3_gpu_tensor *nm = h3_gpu_tensor_from_bf16(gpu, nmod,
                                                MAP_ROWS * SLOTS * WIDTH);
    h3_gpu_tensor *rm = h3_gpu_tensor_from_u32(gpu, row_map, ROWS);
    h3_gpu_tensor *gated_t = h3_gpu_tensor_new_bf16(gpu, ROWS * WIDTH);
    h3_gpu_tensor *qout = h3_gpu_tensor_new_i8(gpu, ROWS * WIDTH);
    h3_gpu_tensor *qsc = h3_gpu_tensor_new_f32(gpu, ROWS);
    CHECK(res && br && w && gm && nm && rm && gated_t && qout && qsc);
    if (!failed) {
        h3_gpu_begin(gpu);
        ...
        if (same) {
             ...
             * driver-dependent slack; the quantized payload is exact. */
            for (size_t index = 0; index < ROWS; index++) {
                float scale = fabsf(got_scales[index]) > 1.0f ?
                    fabsf(got_scales[index]) : 1.0f;
                if (fabsf(got_scales[index] - expected_scales[index]) >
                    1e-6f * scale)
                    same = 0;
            }
        }
        if (!same) {
            int printed = 0;
            for (size_t index = 0; index < ROWS * WIDTH && !printed; index++) {
                if (got[index] != expected[index]) {
                    fprintf(stderr,
                            "  gate_adaln_q[%zu]: got %d expected %d\n",
                            index, got[index], expected[index]);
                    printed = 1;
                }
            }
            if (!printed) {
                for (size_t index = 0; index < ROWS; index++) {
                    if (fabsf(got_scales[index] - expected_scales[index]) >
                        1e-6f) {
                        fprintf(stderr,
                                "  gate_adaln scale[%zu]: got %.9g expected %.9g\n",
                                index, got_scales[index],
    ...
    h3_gpu_tensor_free(br);
    h3_gpu_tensor_free(w);
    h3_gpu_tensor_free(gm);
    h3_gpu_tensor_free(nm);
    h3_gpu_tensor_free(rm);
    h3_gpu_tensor_free(gated_t);
    h3_gpu_tensor_free(qout);
...
static void test_token_pool_adaln(h3_gpu *gpu) {
    ...
    if (!failed) {
        ...
        for (uint32_t row = 0; row < FULL_ROWS; row++) {
            ...
            float inverse = 1.0f / sqrtf(tree_sum256(squared, WIDTH) /
                                         (float)WIDTH + epsilon);
            uint32_t modulation_base = row_map[row] * SLOTS * WIDTH;
            for (uint32_t column = 0; column < WIDTH; column++) {
                float normalized =
                    bf16_value(expected_full[row * WIDTH + column]) *
                    inverse * bf16_value(nw[column]);
                float shift = bf16_value(mod[modulation_base + column]);
                float scale = bf16_value(
                    mod[modulation_base + WIDTH + column]);
...
int main(int argc, char **argv) {
    ...
    test_copy(gpu);
    test_mlp_bf16(gpu);
    test_mlp_nax_bf16(gpu);
    test_grouped_qkv_linear_rope_bf16(gpu);
    test_swiglu_f32(gpu);
    test_video_qkv_rope_f32(gpu);
    test_vae_encoder_pad_f32(gpu);
